...
def get_predicted_volumes(pred):
    ...
    volume_post = np.sum(pred[pred==2])
...
def create_report(inference, header, orig_vol, pred_vol):
   ...
   # another choice would be to find the three maximum labeled hippocampus volume slices are located.
...
if __name__ == "__main__":
    ...
    report_save_path = os.path.join('..', 'out', date+t+'_report.dcm')
